...
int epoll_add_fd(int fd)
{
    ...
    if(ret < 0) {
        printf("epoll_ctl Add fd:%d error, Error:[%d:%s]", fd, errno, strerror(errno));
        return -1;
    }

    printf("epoll add fd:%d--->%d success", fd, g_epollfd);
    return 0;
}
...
int epollfd_init()
{
    ...
    g_epollfd = epfd;
    printf("epoll fd:%d create success", epfd);
...
int timerfd_init()
{
    ...
    if (tmfd < 0) {
        printf("timerfd_create error, Error:[%d:%s]", errno, strerror(errno));
        return -1;
    }

    ret = timerfd_settime(tmfd, 0, &new_value, NULL);
    if (ret < 0) {
        printf("timerfd_settime error, Error:[%d:%s]", errno, strerror(errno));
        close(tmfd);
        return -1;
    }
